...
const televeryModel = app_require('versions/1.0/Model/Televery.js');
...
/* exports.getTeleveryStations = function()
{
  const deferred = Q.defer();
  var stationToRet = [];
	const stationGlobal = dbUtil.getStation3Global();
  const televeryStationList = dbUtil.getTeleveryStation3Global();

  for(var stationKey in stationGlobal)
  {
    if(televeryStationList.includes(stationKey))
    {
      var _televeryStation = new stationModel();
      Object.assign(_televeryStation,stationGlobal[stationKey],{
        "stationId":stationKey,
      });
      stationToRet.push(_televeryStation);
    }
  }
...
/* exports.userHasTeleveryReq = function(_userId)
{
  const deferred = Q.defer();
  const db2 = dbUtil.admin2.database();
  const queueRef = db2.ref("TeleveryReqQueue");
  try {
    queueRef.orderByChild("userId").equalTo(_userId).once("value").then(function(snapshot){
      var exists = snapshot.exists();
      if(!exists)
      {
      ...
      logger.log("error",logger.logErrorReport("ERROR","/1.0/userHasTeleveryReq@1268",[error]));
      deferred.reject();
    });
  } catch (e) {
    logger.log("error",logger.logErrorReport("ERROR","/1.0/userHasTeleveryReq@1272",[e]));
    deferred.reject();
  }
  return deferred.promise;
} */

/* exports.createTeleveryReq = function(_userId,_stationId,_tripId)
{
  const deferred = Q.defer();
  const db = dbUtil.admin.database();
  const db2 = dbUtil.admin2.database();
  ...
  const televeryStationList = dbUtil.getTeleveryStation3Global();
  const televeryOBJ = new televeryModel();
  var _userScooterId = "NIL";
  var _userHasTeleveryPromo = false;
  var _userScooterBatteryId = "NIL";
  var _userFoundAt = "SGP";
  var _stationName = "NIL";
  var _MYSCredits = 0;
  var _SGPCredits = 0;
 try {
   if(!televeryStationList.includes(_stationId))
   {
     throw(logger.logErrorReport("ERROR","/1.0/createTeleveryReq@1297",[_userId,_stationId,_tripId]));
   }
   else {
     queueRef.orderByChild("userId").equalTo(_userId).once("value").then(function(snapshot){
       if(snapshot.exists())
       {
         throw(logger.logErrorReport("ERROR","/1.0/createTeleveryReq@1303",[_userId,_stationId,_tripId]));
       }
       else {
         return dbUtil.stationsRef.child(_stationId).once("value");
       }
     }).then(function(_stationOBJ){
      _stationName = _stationOBJ.child("stationName").exists() ? _stationOBJ.val()["stationName"] : "NIL";
      _stationName = _stationName + '_' + _stationId.replace(/DST-|DPT-/,'');
      return db.ref("Transactions").child(_userId).once("value");
     }).then(function(_transactionObj){
       _transactionObj.forEach(function(_transobj){
         var _key = _transobj.ref.key;
         if(_key.includes('TVRY'))
         {
           _userHasTeleveryPromo = true;
         }
       });
       return dd.getTripObj(_userId,_tripId);
     }).then(function(_tripObj){
       _userScooterId = _tripObj.scooterId;
       _userFoundAt = _tripObj.userFoundAt;
       _MYSCredits = _userFoundAt == "MYS" ? 300 : 0;
       _SGPCredits = _userFoundAt == "SGP" && _userHasTeleveryPromo ? 150 : _userFoundAt == "SGP" ? 300 : 0;
       return dd.getScooterOBJ(_userScooterId);
     }).then(function(_scooterObj){
       _userScooterBatteryId = _scooterObj.BatteryId;
        Object.assign(televeryOBJ,{
          "userId":_userId,
          "tripId":_tripId,
          "scooterId":_userScooterId,
          "batteryIn":_userScooterBatteryId,
          "batteryOut":"NIL",
          "stationId":_stationId,
          "stationName":_stationName,
          "MYSCredits":_MYSCredits,
          "SGPCredits":_SGPCredits,
          "userFoundAt":_userFoundAt
        });
       return queueRef.child(nowTime).update(televeryOBJ);
     }).then(function(){
       deferred.resolve({"status":"OK"});
     }).catch(function(err1){
       logger.log("error",logger.logErrorReport("ERROR","/1.0/createTeleveryReq@1345",[err1]));
       deferred.reject();
     });
   }
 }
 catch (e) {
  logger.log("error",logger.logErrorReport("ERROR","/1.0/createTeleveryReq@1351",[e]));
  deferred.reject();
 }
  return deferred.promise;
}
 */
/* exports.removeTeleveryReqQueue = function(_userId)
{
  const deferred = Q.defer();
  const db = dbUtil.admin2.database();
  const queueRef = db.ref("TeleveryReqQueue");
  try {
    queueRef.orderByChild("userId").equalTo(_userId).once("value").then(function(snapshot){
      var exists = snapshot.exists();
      if(!exists)
      {
        throw(logger.logErrorReport("ERROR","/1.0/removeTeleveryReqQueue@1367",[_userId]));
      }
      else
      {
        snapshot.forEach(function(childSnapShot2)
        {
          var televeryReqTime = childSnapShot2.ref.key;
          queueRef.child(televeryReqTime).remove();
        });
        deferred.resolve({"status":"OK"});
      }
    }).catch(function(error){
      logger.log("error",logger.logErrorReport("ERROR","/1.0/removeTeleveryReqQueue@1379",[error]));
      deferred.reject();
    });
  } catch (e) {
    logger.log("error",logger.logErrorReport("ERROR","/1.0/removeTeleveryReqQueue@1383",[e]));
    deferred.reject();
  }
  return deferred.promise;
} */

/* exports.finTeleveryReq = function(_userId,_batteryOut)
{
  const deferred = Q.defer();
  const dbmain = dbUtil.admin.database();
  const db = dbUtil.admin2.database();
  const _userTeleveryFinTime = new Date().getTime();
  const televeryOBJ = new televeryModel();
  var _userTeleveryReqTime = _userTeleveryFinTime;
  var _innerTeleveryObj = {}
  var _userMYSCredits = 0;
  var _userSGPCredits = 0;
  var _userTripId = "NIL";
  var _userHasPromo = false;
  var _userScooterId = "NIL";
  try {
    const televeryLogRef = db.ref("TeleveryLogs");
    const queueRef = db.ref("TeleveryReqQueue");
    dbmain.ref("Transactions").child(_userId).once("value").then(function(_transactionSS){
      _transactionSS.forEach(function(_transobj){
        var _key = _transobj.ref.key;
        if(_key.includes('TVRY'))
        {
          _userHasPromo = true;
      ...
      return queueRef.orderByChild("userId").equalTo(_userId).once("value");
    }).then(function(snapshot){
      var exists = snapshot.exists();
      if(!exists)
      {
        throw(logger.logErrorReport("ERROR","/1.0/finTeleveryReq@1420",[_userId,_batteryOut]));
      }
      else
      {
        snapshot.forEach(function(childSnapShot2)
        {
          _innerTeleveryObj["userReqTime"] = _userTeleveryReqTime = parseInt(childSnapShot2.ref.key);
          _innerTeleveryObj["userId"] = childSnapShot2.child("userId").exists() ?
            childSnapShot2.val()["userId"] : "NIL";
          _innerTeleveryObj["userFoundAt"] = childSnapShot2.child("userFoundAt").exists() ?
            childSnapShot2.val()["userFoundAt"] : "SGP";
          _innerTeleveryObj["scooterId"] = childSnapShot2.child("scooterId").exists() ?
            childSnapShot2.val()["scooterId"] : "NIL";
          _innerTeleveryObj["tripId"] = _userTripId = childSnapShot2.child("tripId").exists() ?
            childSnapShot2.val()["tripId"] : "NIL";
          _innerTeleveryObj["batteryIn"] = childSnapShot2.child("batteryIn").exists() ?
            childSnapShot2.val()["batteryIn"] : "NIL";
          _innerTeleveryObj["batteryOut"] = _batteryOut;
          _innerTeleveryObj["stationId"] = childSnapShot2.child("stationId").exists() ?
            childSnapShot2.val()["stationId"] : "NIL";
          _innerTeleveryObj["stationName"] = childSnapShot2.child("stationName").exists() ?
            childSnapShot2.val()["stationName"] : "NIL";
          _innerTeleveryObj["MYSCredits"] = _userMYSCredits = childSnapShot2.child("MYSCredits").exists() ?
            parseInt(childSnapShot2.val()["MYSCredits"]) : 0;
          _innerTeleveryObj["SGPCredits"] = _userSGPCredits = childSnapShot2.child("SGPCredits").exists() ?
            parseInt(childSnapShot2.val()["SGPCredits"]) : 0;
          _innerTeleveryObj["refId"] = shortUUID.generate();
        });
        Object.assign(televeryOBJ,_innerTeleveryObj);
        return televeryLogRef.child(_userTeleveryFinTime).update(televeryOBJ);
      }
    }).then(function(){
      return queueRef.child(_userTeleveryReqTime).remove();
    }).then(function(){
      return dd.getTripObj(_userId,_userTripId)
    }).then(function(_tripObj){
      _userScooterId = _tripObj.scooterId;
      if(_userSGPCredits != 0)
      {
        var etcFareToSend = _tripObj.etcFare + _userSGPCredits;
        return dd.setTripOBJ(_userId,_userTripId,{"etcFare":etcFareToSend});
      }
      else {
        var etcFareToSend = _tripObj.etcFare + _userMYSCredits;
        return dd.setTripOBJ(_userId,_userTripId,{"etcFare":etcFareToSend});
      }
    }).then(function(){
      return dd.getScooterOBJ(_userScooterId);
    }).then(function(_scooterObj){
      return dd.setScooterOBJ(_userScooterId,{"BatteryId":_batteryOut})
    }).then(function(){
      deferred.resolve({"status":"OK","userHasPromo":_userHasPromo});
    }).catch(function(error){
      logger.log("error",logger.logErrorReport("ERROR","/1.0/finTeleveryReq@1473",[error]));
      deferred.reject();
    });
  } catch (e) {
    logger.log("error",logger.logErrorReport("ERROR","/1.0/finTeleveryReq@1477",[e]));
    deferred.reject();
  }
  return deferred.promise;
} */

exports.payAll = function(nonce,amount,userId,_accountGroup)
{
	const deferred = Q.defer();
  var checkoutCustomerId;
  var transactionId;
  var amountFireBase = parseInt(parseFloat(amount)*100);
  var userObjCredit;
  var userObjDeposit;
  var userObjTripCount;
  var maskedCreditCard;
  var userCreditCardUID = "NIL";
  var creditCardType;
  var userCreditCardToken;
  var _userFoundAt="NIL";
  var _userContact;
  var promiseToReturn = [];
  const db = dbUtil.admin.database();
  ...
  try {
    ...
    usersRef.once("value").then(function(snapshot){
      ...
      if (!exists)
      {
        throw(logger.logErrorReport("ERROR","/1.0/checkout@1510",[nonce,amount,userId,userFoundAt]));
      }
      else {
        userObjCredit = snapshot.child("SGPCredits").exists() ? parseInt(snapshot.val()["SGPCredits"]) : 0;
        userObjDeposit = snapshot.child("deposit").exists() ? parseInt(snapshot.val()["deposit"]) : 0;
        userObjTripCount = snapshot.child("tripCount").exists() ? parseInt(snapshot.val()["tripCount"]) : 0;
        ...
        var _merchantAccountId = _userFoundAt == "MYS" ? "telepodMYR": amountFireBase <= 1200 ?
      ...
      else
      {
        ...
        throw(logger.logErrorReport("ERROR","/1.0/payAll@2082",[amount,userId,_transactionStatus,
          _transactionResponseCode,_transactionResponseText]));
      }
    }).then(function(transOBJ){
      var transactionOBJ = {};
      transactionOBJ["paymentTime"] = parseInt(new Date().getTime()/1000);
      transactionOBJ["paymentAmount"] = amountFireBase;
      ...
      var userCredit = userObjCredit + amountFireBase;

      if(_userFoundAt == "MYS")
      {
        usersRef.update({"MYSCredits":userCredit,"creditCardType":creditCardType,
          ...
          logger.log("error",logger.logErrorReport("ERROR_DB_WRITING","/1.0/checkout@1582",[err,userId,amount]));
        });
      }
      else {
        usersRef.update({"SGPCredits":userCredit,"creditCardType":creditCardType,
          "creditCardNumber":maskedCreditCard,"creditCardToken":userCreditCardToken,
          "customerId":checkoutCustomerId,"creditCardUID":userCreditCardUID}).then(function(){
            return transactionRef.child(transactionId).update(transactionOBJ);
        }).catch(function(err){
          logger.log("error",logger.logErrorReport("ERROR_DB_WRITING","/1.0/checkout@1591",[err,userId,amount]));
        });
    }
    dd.getUsedUserContact(_userContact,_accountGroup).then(function(_userObjs){
      if(_userObjs != null && _userObjs.length != 0)
      {
        _userObjs.forEach(function(_userobj){
          var _userId = _userobj.userId;
          promiseToReturn.push(dd.payNegativeCredits(userCreditCardToken,_userId));
        });
        return Q.all(promiseToReturn);
      }
      else {
        deferred.resolve({"status":"OK"});
      }
    ...
    deferred.resolve({"status":"OK"});
      ...
      logger.log("error",logger.logErrorReport("ERROR","/1.0/checkout@1612",[err]));
      deferred.reject(err);
  });
  } catch (e) {
    logger.log("error",logger.logErrorReport("ERROR","/1.0/checkout@1616",[e]));
    deferred.reject();
  }
	return deferred.promise;
}
...
/* exports.checkIfLatLngInOperatingZonesFrontEnd = function(userLat,userLng,userId,countryFound,_accountGroup)
{
  const deferred = Q.defer();
  var zoneObjToReturn = {};
  dd.getNearbyZones(userLat,userLng,6,_accountGroup).then(function(zoneArr){
    if(userId !== "NIL")
    {
      dd.setUserOBJ(userId,{"userFoundAt":countryFound},_accountGroup).catch(function(e){
        //do nothing..
      });
    }
    if(zoneArr.length != 0)
    {
     zoneArr.forEach(function(zoneObj){
       ...
       if(_isInside)
       {
         zoneObjToReturn = zoneObj;
       }
     });
     if(zoneObjToReturn == undefined || zoneObjToReturn == null || Object.keys(zoneObjToReturn).length === 0)
     {
       deferred.resolve({});
     }
     else {
       deferred.resolve(zoneObjToReturn);
     }
    }
    else {
      deferred.resolve({});
    ...
    logger.log("error",logger.logErrorReport("ERROR","/1.0/checkIfLatLngInOperatingZonesFrontEnd@1819",[err]));
    deferred.reject();
  });
  return deferred.promise;
} */
...
exports.checkUserReferral = function(userId,refId,_accountGroup)
{
  ...
  // var _userFoundAt = "SGP";
  // var _refUserFoundAt = "SGP";
  var userCredit = 0;
  var userAuthenticated = false;
  var _userPaymentToGive = 0;
  var refUserAuthenticated = false;
  var userContact = -1;
  var refUserContact = -1;
  var refUserCredit = 0;
  var refTripCount = 0;
  var _refUserPaymentToGive = 0;
  var _timenow = new Date().getTime();
  var refRegistrationDate = _timenow;
